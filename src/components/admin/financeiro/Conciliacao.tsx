...
function PainelLinha({
  ...
  const sugestoes = useQuery({
    ...
    enabled: linha.status === "pendente" || linha.status === "parcial" || linha.status === "divergente",
  });
  ...
  const aplicar = useMutation({
    mutationFn: async () => {
      ...
      if (alocacoes.length === 0) throw new Error("Escolha ao menos uma parcela e informe o valor.");
      return conciliar({
        line_ids: [linha.id],
        alocacoes,
  ...
  return (
    <aside className="ledger-panel space-y-5 p-5">
      ...
      {linha.error_reason && <p className="text-sm text-ledger-muted">Motivo: {linha.error_reason}</p>}
      {linha.flag_reason && <p className="text-sm text-ledger-muted">Observação: {linha.flag_reason}</p>}
      ...
      {podeConciliar && (linha.status === "pendente" || linha.status === "divergente") && (
        <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-ledger-muted">
                      ...
                      {formatBRLFromCents(s.aberto_cents)}
                    </p>
                    <p className="text-xs text-ledger-muted">
                      Confiança {s.score}% —{" "}
                      {[
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            <input value={tarifa} onChange={(e) => setTarifa(e.target.value)} placeholder="Tarifa R$" className={inputCls} />
            <input value={juros} onChange={(e) => setJuros(e.target.value)} placeholder="Juros R$" className={inputCls} />
            <input value={desconto} onChange={(e) => setDesconto(e.target.value)} placeholder="Desconto R$" className={inputCls} />
          </div>
          <label className="flex items-center gap-2 text-sm font-medium text-ledger-muted">
            <input type="checkbox" checked={excedente} onChange={(e) => setExcedente(e.target.checked)} />
            Aceitar valor acima do saldo da parcela (excedente explícito)
          </label>
          <p className="text-xs text-ledger-muted">
            A soma alocada precisa ser igual a {formatBRLFromCents(emAberto)} — o valor que entrou ou saiu no banco.
          </p>
          <button
            type="button"
...
export function Conciliacao() {
  ...
  const contas = useQuery({ queryKey: ["fin-accounts"], queryFn: fetchFinAccounts, enabled: podeVer });
  ...
  const linhas = useQuery({
    ...
    queryFn: () => listarLinhasExtrato({ ...filtros, limit: PAGE_SIZE, offset: pagina * PAGE_SIZE }),
    enabled: podeVer && Boolean(conta),
  });
  const resumo = useQuery({
    queryKey: ["fin-extrato-resumo", conta, de, ate],
    queryFn: () => resumoExtrato({ ...(conta ? { financial_account_id: conta } : {}), ...(de ? { de } : {}), ...(ate ? { ate } : {}) }),
    enabled: podeVer && Boolean(conta),
  });
  ...
  const enviar = useMutation({
    ...
    onSuccess: (r) => {
      if (r.repetido) {
        setDiagnostico(`Este arquivo já havia sido importado (impressão digital ${r.sha256.slice(0, 12)}…). Nada foi duplicado.`);
  ...
  const desfazer = useMutation({
    mutationFn: async (linha: LinhaExtratoRow) => {
      ...
      const motivo = window.prompt("Motivo para desfazer a conciliação:")?.trim();
      if (!motivo) throw new Error("Desfazer conciliação exige motivo.");
      return desfazerConciliacao(alvo, motivo);
    },
    onSuccess: () => {
  ...
  const colunas: Column<LinhaExtratoRow>[] = [
    { key: "data", header: "Data", render: (r) => <span className="tabular-nums">{dataBR(r.data)}</span> },
    {
      key: "historico",
      header: "Lançamento",
      render: (r) => (
        <div className="min-w-0">
          <p className="truncate font-semibold text-ledger-text">{r.historico || "Sem histórico"}</p>
    ...
    <div className="space-y-6">
      <Panel title="Extrato bancário">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          />
          <input type="date" value={de} onChange={(e) => setDe(e.target.value)} aria-label="De" className={inputCls} />
          <input type="date" value={ate} onChange={(e) => setAte(e.target.value)} aria-label="Até" className={inputCls} />
          <SmartSelect
            options={[
