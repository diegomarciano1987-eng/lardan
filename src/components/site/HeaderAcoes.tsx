  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      <Link to="/carrinho" aria-label={`Sacola${qtd ? ` com ${qtd} peça(s)` : " vazia"}`} className={base}>
        ...
        {qtd > 0 ? (
          <span className="absolute -right-0.5 -top-0.5 grid h-[1.15rem] min-w-[1.15rem] place-items-center rounded-full bg-primary px-1 text-[0.625rem] font-medium text-primary-foreground">nt-medium text-primary-foreground shadow-[0_6px_16px_-8px_rgba(0,0,0,0.6)]">
            {qtd}
          </span>
        ) : null}
      </Link>
