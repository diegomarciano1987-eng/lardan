...
export function HeroScroll() {
  ...
  const iconOut = ease(phase(p, 0.27, 0.48));
  const wordmarkIn = ease(phase(p, 0.38, 0.58));
  const nevoa = reduced ? 0 : phase(p, 0.29, 0.4) * (1 - phase(p, 0.4, 0.57));
  const mostrarReflexo = !reduced && iconOut < 0.65;

  return (
    <div ref={trackRef} className="relative h-[270vh]" aria-label={BRAND.name}>
