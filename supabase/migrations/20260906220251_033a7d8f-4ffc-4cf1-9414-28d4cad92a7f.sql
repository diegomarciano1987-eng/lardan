CREATE OR REPLACE FUNCTION public.homolog_purge(_prefix text DEFAULT 'HOMOLOG'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  delete from public.stock_movements sm using public.product_variants v, public.products p
    where sm.variant_id = v.id and v.product_id = p.id and p.slug like slug_like;

  delete from public.product_media pm using public.products p
    where pm.product_id = p.id and p.slug like slug_like;

  delete from public.media_assets where alt like txt_like;
