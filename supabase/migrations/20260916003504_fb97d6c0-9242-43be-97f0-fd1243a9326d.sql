create or replace function public.fin_unaccent_lower(_t text)
returns text language sql immutable set search_path = public as $$
  select lower(translate(coalesce(_t,''),
    'ÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇáàâãäéèêëíìîïóòôõöúùûüç',
    'AAAAAEEEEIIIIOOOOOUUUUCaaaaaeeeeiiiiooooouuuuc'))
$$;
revoke all on function public.fin_unaccent_lower(text) from public, anon;
grant execute on function public.fin_unaccent_lower(text) to authenticated;
  ...
  select _line, c.id, c.score, c.reasons from (
    select i.id,
      ...
      + (case when p.nome is not null and coalesce(l.historico,'') <> ''
               and fin_unaccent_lower(l.historico) like '%' || fin_unaccent_lower(split_part(p.nome,' ',1)) || '%' then 20 else 0 end)
      as score,
...
drop policy if exists "extratos leitura" on storage.objects;
create policy "extratos leitura" on storage.objects for select to authenticated
  using (bucket_id = 'extratos-bancarios' and has_capability(auth.uid(),'finance.statement.view'));
drop policy if exists "extratos envio" on storage.objects;
create policy "extratos envio" on storage.objects for insert to authenticated
  with check (bucket_id = 'extratos-bancarios' and has_capability(auth.uid(),'finance.statement.import'));