...
-- ---------------------------------------------------------------- leitores
create or replace function public.imp_valor_cents(_v text)
returns bigint language plpgsql immutable as $$
declare s text; sinal int := 1;
begin
  s := trim(coalesce(_v,''));
  if s = '' then return null; end if;
  if s like '(%)' then sinal := -1; s := trim(both '()' from s); end if;
  if s like '-%' then sinal := -1; end if;
  ...
  return sinal * round(s::numeric * 100)::bigint;
exception when others then return null;
end $$;

create or replace function public.imp_data(_v text)
returns date language plpgsql immutable as $$
declare s text; a int; m int; d int; p text[];
begin
  s := trim(coalesce(_v,''));
  if s = '' then return null; end if;
  s := split_part(s, ' ', 1);
  if s ~ '^[0-9]{4}-[0-9]{1,2}-[0-9]{1,2}$' then
    p := string_to_array(s, '-'); a := p[1]::int; m := p[2]::int; d := p[3]::int;
  elsif s ~ '^[0-9]{1,2}[/\.-][0-9]{1,2}[/\.-][0-9]{2,4}$' then
    p := regexp_split_to_array(s, '[/\.-]'); d := p[1]::int; m := p[2]::int; a := p[3]::int;
    if a < 100 then a := 2000 + a; end if;
  else
    return null;
  end if;
  if m < 1 or m > 12 or d < 1 or d > 31 or a < 1900 or a > 2200 then return null; end if;
  return make_date(a, m, d);
exception when others then return null;
end $$;

create or replace function public.imp_texto(_v text)
returns text language sql immutable as $$ select nullif(btrim(coalesce(_v,'')), '') $$;

-- ------------------------------------------------------- importar pessoas
create or replace function public.import_pessoas(
  _rows jsonb, _papel text, _dry_run boolean default true)
...
declare
  ...
  v_linhas jsonb := '[]'::jsonb; v_existente record; v_mudou boolean;
begin
  ...
  for r in select * from jsonb_array_elements(coalesce(_rows,'[]'::jsonb)) loop
    ...
    if v_sit is null then
      -- identidade: documento, depois contato
      if v_doc is not null then
        select p.id, p.display_name, p.birth_date, p.profession, p.notes into v_existente
        from parties p where p.doc_digits = v_doc limit 1;
      end if;
      if v_existente.id is null and (v_email is not null or v_fone_norm is not null) then
        select p.id, p.display_name, p.birth_date, p.profession, p.notes into v_existente
      ...
      if v_existente.id is null and v_doc is null and v_email is null and v_fone_norm is null then
        -- só nome: pode ser homônimo; não adivinha
        if exists (select 1 from parties p where lower(unaccent(coalesce(p.display_name,''))) = lower(unaccent(v_nome))) then
          v_sit := 'pendente'; v_motivo := 'Já existe cadastro com este nome e a linha não trouxe documento nem contato.';
        end if;
      end if;
    end if;
    ...
    if v_sit is null then
      if v_existente.id is not null then
        v_id := v_existente.id;
        if v_doc is not null and v_existente.display_name is not null
           and lower(unaccent(v_existente.display_name)) <> lower(unaccent(v_nome)) then
          v_sit := 'conflito';
          v_motivo := 'Documento já cadastrado para "' || v_existente.display_name || '".';
        else
          v_mudou := false;
          if not _dry_run then
            update parties p set
              ...
              updated_by = v_uid
            where p.id = v_id
              and (p.birth_date is distinct from coalesce(p.birth_date, v_nasc)
                or p.profession is distinct from coalesce(p.profession, v_prof)
                or p.notes is distinct from coalesce(p.notes, v_obs));
            if found then v_mudou := true; end if;
          else
            v_mudou := (v_existente.birth_date is null and v_nasc is not null)
                    or (v_existente.profession is null and v_prof is not null)
                    or (v_existente.notes is null and v_obs is not null);
          end if;
    ...
    -- contatos (somente quando há pessoa gravada)
  ...
  if not _dry_run then
    insert into audit_logs (table_name, record_id, action, actor_id, diff)
    values ('parties', gen_random_uuid(), 'import_pessoas', v_uid,
...
revoke all on function public.import_pessoas(jsonb, text, boolean) from public, anon;
grant execute on function public.import_pessoas(jsonb, text, boolean) to authenticated;

-- ------------------------------------------------------- importar títulos
create or replace function public.import_titulos(
  _rows jsonb, _direction text, _dry_run boolean default true,
  _criar_contraparte boolean default false)
returns jsonb language plpgsql security definer set search_path = public as $$
declare
...
begin
  ...
  for r in select * from jsonb_array_elements(coalesce(_rows,'[]'::jsonb)) loop
    ...
    if v_sit is null then
      ...
      if v_party is null and v_nome is not null then
        ...
        where lower(unaccent(coalesce(p.display_name,''))) = lower(unaccent(v_nome)) limit 1;
      end if;
    ...
    if v_sit is null then
      v_chave := coalesce(imp_texto(r->>'id_externo'),
        md5(_direction || '|' || coalesce(v_doc, lower(unaccent(coalesce(v_nome,'')))) || '|' ||
  ...
  if not _dry_run then
    insert into audit_logs (table_name, record_id, action, actor_id, diff)
    values ('financial_titles', gen_random_uuid(), 'import_titulos', v_uid,
...
revoke all on function public.import_titulos(jsonb, text, boolean, boolean) from public, anon;
grant execute on function public.import_titulos(jsonb, text, boolean, boolean) to authenticated;
